...
describe("resolveNodeCommandAllowlist", () => {
  it("includes iOS service commands by default", () => {
    ...
    expect(allow.has("calendar.update")).toBe(false);
    expect(allow.has("reminders.update")).toBe(false);
    expect(allow.has("calendar.delete")).toBe(false);
    expect(allow.has("reminders.delete")).toBe(false);
  });
  ...
  it("can explicitly allow dangerous commands via allowCommands", () => {
    const allow = resolveNodeCommandAllowlist(
      {
        gateway: {
          nodes: {
            allowCommands: ["camera.snap", "screen.record", "calendar.update", "reminders.update"],
          },
        },
      },
    ...
    expect(allow.has("calendar.update")).toBe(true);
    expect(allow.has("reminders.update")).toBe(true);
    expect(allow.has("camera.clip")).toBe(false);
    expect(allow.has("calendar.add")).toBe(false);
    expect(allow.has("reminders.add")).toBe(false);
  });
});
