...
const CALENDAR_DANGEROUS_COMMANDS = ["calendar.add", "calendar.update"];

const REMINDERS_COMMANDS = ["reminders.list"];
const REMINDERS_DANGEROUS_COMMANDS = ["reminders.add", "reminders.update"];
...
function normalizePlatformId(platform?: string, deviceFamily?: string): string {
  ...
  if (raw.startsWith("mac") || raw.startsWith("darwin")) {
    return "macos";
  }
